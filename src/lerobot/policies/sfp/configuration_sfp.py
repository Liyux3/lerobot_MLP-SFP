...
class SFPConfig(PreTrainedConfig):
    ...
    def __post_init__(self):
        ...
        if self.horizon <= self.n_obs_steps:
            raise ValueError(
                f"horizon must be greater than n_obs_steps, got {self.horizon=} and {self.n_obs_steps=}"
            )
        if not 1 <= self.n_action_steps <= self.horizon - self.n_obs_steps:
            raise ValueError(
                "n_action_steps must fit in the integration intervals, got "
                f"{self.n_action_steps=} and {self.horizon - self.n_obs_steps=}"
            )
        if self.integration_steps < 1:
            raise ValueError(f"integration_steps must be positive, got {self.integration_steps}")
        if self.k <= 0:
            raise ValueError(f"k must be positive, got {self.k}")
        if self.sigma_0 < 0:
            raise ValueError(f"sigma_0 must be non-negative, got {self.sigma_0}")

        valid_types = (
            "bottleneck_skip",
            "bottleneck_skip_auto",
            "pure_mlp",
            "cond_every_layer",
            "cond_residual",
            "unet",
